import Image from "next/image";
import Link from "next/link";

import {
  buildOrderWhatsAppMessage,
  buildWhatsAppHref,
  type MomoContact,
} from "@/lib/contact";
import { getMomoNetworkLogo } from "@/lib/momo-networks";
import { formatPrice } from "@/lib/utils";

type OrderItemSummary = {
  name: string;
  quantity: number;
  price: number;
};

type MomoPaymentInstructionsProps = {
  orderCode: string;
  total: number;
  momoContact: MomoContact | null;
  whatsappNumber: string;
  customerName: string;
  customerPhone: string;
  items: OrderItemSummary[];
  variant?: "default" | "ticket";
};

export function MomoPaymentInstructions({
  orderCode,
  total,
  momoContact,
  whatsappNumber,
  customerName,
  customerPhone,
  items,
  variant = "default",
}: MomoPaymentInstructionsProps) {
  const totalLabel = formatPrice(total);
  const whatsappHref = buildWhatsAppHref(
    whatsappNumber,
    buildOrderWhatsAppMessage({
      orderCode,
      totalLabel,
      customerName,
      customerPhone,
      items,
      momoNetwork: momoContact?.network,
      momoNumber: momoContact?.number,
    }),
  );

  const isTicket = variant === "ticket";

  return (
    <div
      className={
        isTicket
          ? "space-y-3"
          : "space-y-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4"
      }
    >
      {!isTicket ? (
        <div>
          <p className="text-sm font-semibold text-amber-900">Paiement a effectuer</p>
          <p className="mt-1 text-2xl font-black text-amber-950">{totalLabel}</p>
        </div>
      ) : (
        <p className="text-center text-sm font-semibold text-chili">
          A payer · {totalLabel}
        </p>
      )}

      <div
        className={
          isTicket
            ? "rounded-lg border border-dashed border-chili/35 bg-chili/[0.04] px-3 py-3 text-sm text-[#13241c]"
            : "rounded-xl border border-amber-200/80 bg-white/70 px-3 py-3 text-sm text-amber-950"
        }
      >
        {momoContact ? (
          <>
            <p>
              Deposez <strong>{totalLabel}</strong> sur ce numero MoMo :
            </p>
            <div className="mt-3 flex items-center gap-3 text-lg font-black tracking-wide">
              <Image
                src={getMomoNetworkLogo(momoContact.network)}
                alt={momoContact.network}
                width={40}
                height={40}
                className="h-10 w-10 shrink-0 rounded-lg object-contain"
              />
              <span className="min-w-0">
                <span
                  className={`block text-xs font-semibold uppercase tracking-wide ${
                    isTicket ? "text-palm/60" : "text-amber-800/80"
                  }`}
                >
                  {momoContact.network}
                </span>
                {momoContact.number}
              </span>
            </div>
          </>
        ) : (
          <p>
            Aucun numero MoMo n&apos;est associe a cette commande. Contactez le restaurant par
            WhatsApp.
          </p>
        )}
        <p className={`mt-3 ${isTicket ? "text-palm/60" : "text-ink-muted"}`}>
          Envoyez ensuite la capture du paiement et le code{" "}
          <strong>{orderCode}</strong> sur WhatsApp.
        </p>
      </div>

      <Link
        href={whatsappHref}
        target="_blank"
        rel="noreferrer"
        className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#1ebe57]"
      >
        <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor">
          <path d="M20.52 3.48A11.88 11.88 0 0 0 12.04 0C5.47 0 .12 5.34.12 11.9c0 2.1.55 4.16 1.6 5.97L0 24l6.32-1.66a11.84 11.84 0 0 0 5.72 1.46h.01c6.57 0 11.92-5.35 11.92-11.9 0-3.18-1.24-6.16-3.45-8.42ZM12.05 21.8h-.01a9.86 9.86 0 0 1-5.03-1.38l-.36-.21-3.75.98 1-3.65-.23-.37a9.9 9.9 0 0 1-1.52-5.27c0-5.47 4.45-9.91 9.92-9.91 2.65 0 5.15 1.03 7.02 2.9a9.84 9.84 0 0 1 2.9 7 9.92 9.92 0 0 1-9.94 9.91Zm5.44-7.4c-.3-.15-1.78-.88-2.06-.98-.28-.1-.48-.15-.68.15-.2.3-.78.97-.96 1.16-.18.2-.35.22-.65.08-.3-.15-1.27-.47-2.42-1.5-.9-.8-1.5-1.78-1.68-2.08-.18-.3-.02-.46.13-.6.13-.13.3-.35.45-.53.15-.17.2-.3.3-.5.1-.2.05-.38-.02-.53-.08-.15-.68-1.64-.94-2.24-.25-.6-.5-.5-.68-.5h-.58c-.2 0-.53.07-.8.37-.28.3-1.06 1.03-1.06 2.5 0 1.48 1.08 2.9 1.23 3.1.15.2 2.12 3.23 5.14 4.54.72.31 1.28.5 1.72.63.72.23 1.38.2 1.9.12.58-.09 1.78-.73 2.03-1.43.25-.7.25-1.3.17-1.42-.07-.12-.27-.2-.57-.35Z" />
        </svg>
        Envoyer sur WhatsApp
      </Link>
    </div>
  );
}
