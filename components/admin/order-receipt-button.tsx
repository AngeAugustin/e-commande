...
export function OrderReceiptButton({ order, labeled = false }: OrderReceiptButtonProps) {
  ...
  if (!paid) {
    if (labeled) {
      return (
        <Button variant="secondary" disabled title="Recu disponible apres confirmation du paiement">
          Recu indisponible
        </Button>
      );
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-palm-deep/45 p-4">
          <Card className="max-h-[90vh] w-full max-w-lg overflow-y-auto">
            <div className="mb-3 flex items-center justify-between gap-2">
              <h3 className="text-lg font-bold text-palm">Apercu ticket</h3>
